    @if ($href)
        <a href="{{ $href }}" class="btn btn-{{ $size }} btn-{{ $type }}"
    ...
    @else
        <button type="{{ $typeButton ?? 'button' }}"
            class="btn btn-{{ $size }} btn-{{ $type }} text-nowrap"
            @if ($wireClick) wire:click="{{ $wireClick }}" @endif
            @if ($tooltip) data-bs-toggle="tooltip" title="{{ $tooltip }}"
        @elseif ($dataBsToogle)
            data-bs-toggle="{{ $dataBsToogle }}" @endif
            ...
            @if ($loading && $loadingTarget)
                ...
                <span wire:loading wire:target="{{ $loadingTarget }}">
                    <span class="spinner-border spinner-border-sm @if ($text) me-2 @endif"
                        role="status"></span>
                    @if ($text)
                        Chargement...
                    @endif
